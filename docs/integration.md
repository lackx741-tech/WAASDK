...
> **Peer dependencies** — `ethers@^6` is required. `viem`, `wagmi`, and `@reown/appkit` are needed only if you use the wallet-connection module (`sdk/wallet.js`). Install them if you want the full WalletConnect flow:
> ```bash
